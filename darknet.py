...
def detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.6, debug= False):
    """
...
def detect_image(net, meta, im, thresh=.5, hier_thresh=.5, nms=.6, debug= False):
    #import cv2
    ...
    for j in range(num):
        ...
        for i in range(meta.classes):
            ...
            if dets[j].prob[i] > 0:
                #print('meta.names[0]', meta.names[1])
                b = dets[j].bbox
                if altNames is None:
                    #nameTag = meta.names[i]
                    nameTag = i+1
...
def convertBack(x, y, w, h):
    ...
    return xmin if xmin>=0 else 0, ymin, xmax, ymax
def xywh2xyxy(x): 
    y = x.new(x.shape)  
    y[..., 0] = x[..., 0] - x[..., 2] / 2
    y[..., 1] = x[..., 1] - x[..., 3] / 2
    ...
    y[..., 3] = x[..., 1] + x[..., 3] / 2
    return y
#"/home/AirsZXG/yolov5-new-nnie/data/JC/2009_val.txt"
#"/home/AirsZXG/yolov5-new-nnie/data/JC/test2/train.txt"
def performDetect(imagePath="/home/AirsZXG/yolov5-new-nnie/data/JC/test2/train.txt", thresh= 0.25, configPath = "cfg/yolov4-custom.cfg", weightPath = "/home/AirsZXG/darknet-AB/objbackup/yolov4-custom_best.weights", metaPath= "data/obj.data", showImage= False, makeImageOnly = False, initOnly= False):
    """
    ...
    for line in open(imagePath,"r"):
        line=line.strip('\n')
        filepath,fullflname = os.path.split(line)
        detections = detect(netMain, metaMain, (line).encode("ascii"), thresh)
        ...
        with open('test_result.txt', 'a') as f:
            ...
            for detection in detections:
                ...
                bounds = detection[2]
                xCoord,yCoord,xEntent,yEntent = bounds[0],bounds[1],bounds[2],bounds[3]
                xmin, ymin, xmax, ymax = convertBack(xCoord,yCoord,xEntent,yEntent)
